...
public:
	void on_enter() {
		...
		for (int i = 1; i < lattice_num; ++i) {
			knapsack_lattice_1P_pos[i].x = knapsack_lattice_1P_pos[i - 1].x + img_knapsack_lattice.getwidth();
			knapsack_lattice_1P_pos[i].y = knapsack_lattice_1P_pos[i - 1].y;
			knapsack_lattice_2P_pos[i].x = knapsack_lattice_2P_pos[i - 1].x + img_knapsack_lattice.getwidth();
			knapsack_lattice_2P_pos[i].y = knapsack_lattice_2P_pos[i - 1].y;
			if (i % 5 == 0) {
				knapsack_lattice_1P_pos[i].x = knapsack_lattice_1P_pos[0].x;
				knapsack_lattice_1P_pos[i].y += img_knapsack_lattice.getheight();
				knapsack_lattice_2P_pos[i].x = knapsack_lattice_2P_pos[0].x;
				knapsack_lattice_2P_pos[i].y += img_knapsack_lattice.getheight();
			}
		}
	...
	void on_draw() {
		...
		for (int i = 0; i < lattice_num; ++i) {
			putimage_alpha(knapsack_lattice_1P_pos[i].x, knapsack_lattice_1P_pos[i].y, &img_knapsack_lattice);
			putimage_alpha(knapsack_lattice_2P_pos[i].x, knapsack_lattice_2P_pos[i].y, &img_knapsack_lattice);
		}
...
private:
	POINT background_base_pos = { 0 };
	POINT knapsack_field_background_1P_pos = { 0 };
	POINT knapsack_field_background_2P_pos = { 0 };
	POINT player_1P_pos = { 0 };
	POINT player_2P_pos = { 0 };
	POINT knapsack_1P_pos = { 0 };
	POINT knapsack_2P_pos = { 0 };
	POINT knapsack_lattice_1P_pos[lattice_num] = { 0 };
	POINT knapsack_lattice_2P_pos[lattice_num] = { 0 };
	POINT return_btn_pos = { 0 };
